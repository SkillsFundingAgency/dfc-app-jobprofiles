...
namespace DFC.App.JobProfile.ProfileService.UnitTests.ProfileServiceTests
{
    ...
    public class ProfileServiceRefreshAllSegmentsTests
    {
        ...
        [Fact]
        public async Task JobProfileServiceRefreshAllSegmentsReturnsArgumentNullExceptionWhenNullParamIsUsedAsync()
        {
            // arrange
            var mapper = A.Fake<IMapper>();
            var logService = A.Fake<ILogService>();
            ...
            // act
            var exceptionResult = await Assert.ThrowsAsync<ArgumentNullException>(async () => await jobProfileService.RefreshAllSegments(null, 100, 0).ConfigureAwait(false)).ConfigureAwait(false);
