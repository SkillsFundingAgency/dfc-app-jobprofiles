...
namespace DFC.App.JobProfile.UnitTests.ControllerTests.ProfileControllerTests
{
    ...
    public class ProfileControllerPostRefreshApprenticeshipTests : BaseProfileController
    {
        [Theory]
        ...
        public async Task ProfileControllerPostRefreshApprenticeshipsReturnsSuccessForUpdate(string mediaTypeName)
        {
            ...
            A.CallTo(() => FakeJobProfileService.RefreshApprenticeshipsAsync(A<string>.Ignored, first, skip)).Returns(true);

            // Act
            var result = await controller.RefreshApprenticeships(refreshJobProfileSegmentModel);

            // Assert
            A.CallTo(() => FakeJobProfileService.RefreshApprenticeshipsAsync(A<string>.Ignored, first, skip)).MustHaveHappenedOnceExactly();
        ...
        public async Task ProfileControllerPostRefreshApprenticeshipReturnsBadResultWhenModelIsNull(string mediaTypeName)
        {
            ...
            // Act
            var result = await controller.RefreshApprenticeships(refreshJobProfileSegmentModel).ConfigureAwait(false);
