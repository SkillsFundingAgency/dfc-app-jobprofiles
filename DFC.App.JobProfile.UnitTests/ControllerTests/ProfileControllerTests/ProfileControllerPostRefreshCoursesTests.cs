...
namespace DFC.App.JobProfile.UnitTests.ControllerTests.ProfileControllerTests
{
    ...
    public class ProfileControllerPostRefreshCoursesTests : BaseProfileController
    {
        [Theory]
        ...
        public async Task ProfileControllerPostRefreshApprenticeshipsReturnsSuccessForUpdate(string mediaTypeName)
        {
            ...
            A.CallTo(() => FakeJobProfileService.RefreshCourses(A<string>.Ignored, first, skip)).Returns(true);

            // Act
            var result = await controller.RefreshCourses(refreshJobProfileSegmentModel);

            // Assert
            A.CallTo(() => FakeJobProfileService.RefreshCourses(A<string>.Ignored, first, skip)).MustHaveHappenedOnceExactly();
        ...
        public async Task ProfileControllerPostRefreshApprenticeshipReturnsBadResultWhenModelIsNull(string mediaTypeName)
        {
            ...
            // Act
            var result = await controller.RefreshCourses(refreshJobProfileSegmentModel).ConfigureAwait(false);
